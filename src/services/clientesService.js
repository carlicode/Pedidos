...
/**
 * Carga clientes desde Google Sheets CSV
...
export const loadClientes = async () => {
  ...
  if (!csvUrl) {
    throw new Error('URL de clientes no configurada')
  }
  ...
  const clientesUnicos = [...new Set(empresasNombres)] // Remover duplicados
...
/**
 * Carga empresas desde Google Sheets para Agregar Nuevo
 * @returns {Promise<Array>} Array de empresas
 * @throws {Error} Si falla la carga
 */
export const loadEmpresas = async () => {
  const data = await loadFromEnvCSV('VITE_EMPRESAS_CSV_URL')
  return data
}
