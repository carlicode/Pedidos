...
import { formatToStandardDate, prepareDateForSheet, normalizeOrderDate, formatDateForDisplay, convertToISO, getCurrentBoliviaDateStandard } from '../services/dateService.js'
...
export default function Orders() {
  ...
  const [viewType, setViewType] = useState('day') // 'all', 'day' o 'range'
...
const [busquedaBiker, setBusquedaBiker] = useState('')
  ...
  useEffect(() => {
    ...
    console.log('📊 [useEffect] Estado de carga:', {
      activeTab,
      needsData,
      dataLoaded,
  ...
  const handleChange = async (e) => {
    ...
    if (name === 'fecha' && value) {
      console.log('📅 [handleChange] Cambio de fecha detectado:', { name, value })
      const diaSemana = calculateDayOfWeek(value)
      updatedForm.dia_semana = diaSemana
      console.log('📅 [handleChange] Día de semana calculado:', diaSemana)
    }
    ...
    setForm((prev) => {
      ...
      console.log('📝 [handleChange] Form actualizado:', { 
        campo: name, 
        valorAnterior: prev[name], 
        valorNuevo: value,
        fechaActual: newForm.fecha,
        fechaCompleta: newForm
      })
      return newForm
    })
  ...
  const saveToSheet = async (order, silent = false) => {
    console.log('💾 [saveToSheet] Iniciando guardado:', {
      orderId: order.id,
      fecha: order.fecha,
  ...
  const handleAdd = async (e) => {
    ...
    try {
      if (editingOrder) {
        ...
        console.log('📅 [handleAdd] Fecha normalizada:', {
  ...
  const loadOrdersFromSheet = async (forceReload = false) => {
    console.log('🎯 [loadOrdersFromSheet] Iniciando...', { forceReload, loading, dataLoaded, ordersLength: orders.length })
    ...
    try {
      ...
      console.log('🔗 Llamando a loadOrdersAPI()...')
      
      const data = await loadOrdersAPI()
      console.log('📦 Datos recibidos del API:', data?.length || 0, 'pedidos')
      console.log('📦 Tipo de datos:', typeof data, Array.isArray(data) ? 'es array' : 'NO es array')
      ...
      if (data && data.length > 0) {
        ...
        // Log de fechas para debug (Fechas = columna filtrada; Fecha Registro = creación)
        const fechasCol = imported.map(o => o.fechas || o['Fechas']).filter(f => f)
        const fechasUnicas = [...new Set(fechasCol)]
        console.log('📅 [Carga] Columna Fechas en pedidos:', fechasUnicas.slice(0, 10).join(', '), '| total con Fechas:', fechasCol.length)
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (viewType === 'all') {
      // Mostrar todos los pedidos sin filtrar por fecha
      console.log('📋 Mostrando TODOS los pedidos:', filtered.length)
    } else if (viewType === 'day' && dateFilter) {
      // Vista por día: filtrar por columna 'Fechas' (fecha programada del pedido)
      // Aceptar tanto o.fechas (mapeado) como o['Fechas'] (tal cual viene del API)
      const sampleDates = filtered.slice(0, 10).map(o => ({
        id: o.id,
        fechas: o.fechas,
        Fechas: o['Fechas'],
        usado: o.fechas || o['Fechas'] || '(vacío)'
      }))
      console.log('🔍 [Filtro Fechas] dateFilter:', dateFilter, '| muestra:', sampleDates)
      
      filtered = filtered.filter((o) => {
        const fechaProgramada = (o.fechas || o['Fechas'] || '').toString().trim()
        if (!fechaProgramada) return false
        try {
          const orderDate = convertToISO(fechaProgramada)
          if (!orderDate) return false
          const match = orderDate === dateFilter
          if (filtered.indexOf(o) < 3) {
            console.log('🔍 [Filtro Fechas]', { id: o.id, fechaProgramada, orderDate, dateFilter, match })
          }
          return match
        } catch (error) {
          return false
      ...
      console.log('📊 [Filtro Fechas] Resultado:', filtered.length, 'pedidos para', dateFilter)
    } else if (viewType === 'range' && dateRange.start && dateRange.end) {
      // Vista por rango: filtrar por columna 'Fechas' (fecha programada del pedido)
      filtered = filtered.filter((o) => {
        const fechaProgramada = (o.fechas || o['Fechas'] || '').toString().trim()
        if (!fechaProgramada) return false
        try {
                  ...
                  <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setDateFilter(`${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`)
                      }}
                      style={{
                        padding: '8px 12px',
                        border: '1px solid #ddd',
                  ...
                  >
                    ...
                    {filteredOrders.filter(o => o.estado === estado).length === 0 && (
                      <div className="empty-column">
                          ...
                          {viewType === 'day' && filteredOrders.length === 0 && orders.length > 0 && (
                            <div style={{ 
                              marginTop: '12px', 
                              padding: '12px',
                              backgroundColor: '#fff3cd',
                              ...
                              <div style={{ fontWeight: '600', marginBottom: '4px' }}>
                                📅 No hay pedidos para {new Date(dateFilter + 'T00:00:00').toLocaleDateString('es-BO', { day: '2-digit', month: 'long', year: 'numeric' })}
                              </div>
                              <div style={{ fontSize: '12px' }}>
                                Usa las flechas ◀️ ▶️ para navegar entre días o el botón 📅 para seleccionar una fecha
                              </div>
                            </div>
                          )}
                      </div>
